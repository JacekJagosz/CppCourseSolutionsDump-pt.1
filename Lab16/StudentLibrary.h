...
// ----------------------------------------------------------------------------
void OpenStudentsResultsFile(FILE** m_fptr, const char* m_file){
if ((*m_fptr = fopen(m_file,"w")) == NULL){
      printf("[Error]:: opening file: %s\n",m_file);
      exit(1);
  }
}
...
// ----------------------------------------------------------------------------
void PrintStudentsAverageGrade(const Student* m_students){

  for(int i=0;i<NUMBER_OF_STUDENTS;++i){
    //double average = 0; // GetStudentAverageGrade(...);
    printf("%s %s grades average: %.1lf\n", m_students[i].Name, m_students[i].Surname,GetStudentAverageGrade(&m_students[i]));
    }
}
// ----------------------------------------------------------------------------
void WriteStudent(FILE *fp, const Student* m_student){
    if(m_student!=NULL){
        fprintf(fp, "[INFO]:ID \t%d\n", m_student->ID);
        fprintf(fp, "[INFO]:Name\t%s\n", m_student->Name);
        fprintf(fp, "[INFO]:Surname\t%s\n", m_student->Surname);
        fprintf(fp, "[INFO]:Year\t%d\n\n", m_student->Year);
    }
}

// ----------------------------------------------------------------------------
void WriteStudentsData(FILE *fp, const Student* m_students){
    for(int i=0;i<NUMBER_OF_STUDENTS;++i){
        fprintf(fp, "--------------------------\n");
        WriteStudent(fp, &m_students[i]);
    }
}

// ----------------------------------------------------------------------------
int NameCmpr(const void* a, const void* b){
	const Student *s1 = a;
	const Student *s2 = b;
	return strcmp(s1->Name, s2->Name);
}
// ----------------------------------------------------------------------------
int SurnameCmpr(const void* a, const void* b){
	const Student *s1 = a;
	const Student *s2 = b;
	return strcmp(s1->Surname, s2->Surname);
}
// ----------------------------------------------------------------------------
int AverageCmpr(const void* a, const void* b){
	//printf("%lf", GetStudentAverageGrade(a) - GetStudentAverageGrade(b));
	double dif = GetStudentAverageGrade(a) - GetStudentAverageGrade(b);
	int res;
	if(dif > 0.) res=1;
	else if (dif < 0) res=-1;
	else res=0; //Because we operate on double this will most likely won't happen if number of grades is different
	return res;
}
// ----------------------------------------------------------------------------
void SortStudents(Student* m_students, int (*cmp)(const void*, const void*)){
	qsort(m_students, NUMBER_OF_STUDENTS, sizeof(m_students[0]), cmp);
}

// ----------------------------------------------------------------------------
void WriteResult(FILE *fp, const Student* m_student){
	fprintf(fp, "%s %s", m_student->Name, m_student->Surname);
	for (int i=0; i<NMAXGRADES; i++)
		fprintf(fp, " %.1lf", m_student->Grades[i]);
	fprintf(fp, "\n");
}

// ----------------------------------------------------------------------------
void WriteResults(FILE *fp, const Student* m_student){
	for (int i=0; i<NUMBER_OF_STUDENTS; i++)
		WriteResult(fp, &m_student[i]);
}
